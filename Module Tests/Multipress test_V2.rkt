...
;solution works (note some buttons are mutually exclusive
(define Clock (new timer%
                  [notify-callback (λ ()
                                     
                                       (when(key-down? #\space) (set! color1 0) (send gl_enviroment on-paint))
                                       (unless (key-down? #\space)(set! color1 1) (send gl_enviroment on-paint))
                                       (when(key-down? #\f) (set! color2 0) (send gl_enviroment on-paint))
                                       (unless (key-down? #\f) (set! color2 1) (send gl_enviroment on-paint))
                                     (when(key-down? #\s) (set! color3 0) (send gl_enviroment on-paint))
                                       (unless (key-down? #\s) (set! color3 1) (send gl_enviroment on-paint))
                                     (when(key-down? #\a) (set! color4 0) (send gl_enviroment on-paint))
                                       (unless (key-down? #\a) (set! color4 1) (send gl_enviroment on-paint))
                                     (when(key-down? #\d) (set! color5 0) (send gl_enviroment on-paint))
                                       (unless (key-down? #\d) (set! color5 1) (send gl_enviroment on-paint)))]
                   [interval 30]))
